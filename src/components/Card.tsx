...
function Card(item: CardProps) {
    ...
    return (
        <a href={item.link}>
            <motion.div className="shadow-lg  h-[260px] rounded-2xl relative border mt-6 hover:shadow-xl"  initial={{ opacity: 0, scale:0 }}
                    ...
                    viewport={{ once: true }}>
                <div className="w-full h-full bg-cover bg-center rounded-xl absolute " style={{ backgroundImage: `url(${item.image_url ? item.image_url : placeHolder})` }} ></div>
    ...
    bg-clip-padding backdrop-filter backdrop-blur-2xl bg-opacity-70">
                        ...
                        <p className="text-sm font-light">{truncateDescription(item.description, item.image_url ? 120 : 280)}</p>
                    </div>
                </div>
            </motion.div>
        </a>
